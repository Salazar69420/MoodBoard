...
export function EditNoteNode({ note, zoomScale = 1, autoFocus }: Props) {
    ...
    const category = EDIT_CATEGORIES.find(c => c.id === note.categoryId)!;
    ...
    // Zone styling - KEEP = green-ish accent, CUT = red-ish accent
    const isKeep = category.zone === 'keep';
    const zoneBadgeColor = isKeep ? '#4ade80' : '#f87171';
    const zoneBadgeBg = isKeep ? 'rgba(74,222,128,0.1)' : 'rgba(248,113,113,0.1)';
    const zoneBadgeBorder = isKeep ? 'rgba(74,222,128,0.25)' : 'rgba(248,113,113,0.25)';
    const zoneLabel = isKeep ? 'KEEP' : 'CUT';
